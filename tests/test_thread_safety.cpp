...
static void test_dma_channel_external_lock() {
    ...
    std::thread t1([&]{
        for (int i = 0; i < ITERS; i++) {
            std::lock_guard<std::recursive_mutex> lk(ch.get_mutex());
            shared_counter++;
        }
    });
...
static void test_dma_try_lock_fallback() {
    ...
    std::unique_lock<std::recursive_mutex> held(ch.get_mutex());
...
// ---------------------------------------------------------------------------
// 12. DMAChannel::pull_data() on an inactive channel returns NoMoreData.
//     (pull_data uses a blocking lock, not try_lock, so the audio callback
//     always gets data or silence — it never stops the cubeb stream mid-chime
//     due to spurious lock contention.)
// ---------------------------------------------------------------------------

static void test_dma_pull_data_inactive_channel() {
    cout << "  test_dma_pull_data_inactive_channel..." << endl;

    DMAChannel ch("test-dma-pulldata-inactive");

    // Channel is never started — pull_data should immediately report no data.
    uint32_t avail = 999;
    uint8_t *p_data = nullptr;
    DmaPullResult result = ch.pull_data(1024, &avail, &p_data);

    TEST_ASSERT(result == DmaPullResult::NoMoreData,
                "pull_data on inactive channel should return NoMoreData");
    TEST_ASSERT(avail == 0,
                "pull_data on inactive channel should set avail_len=0");
}
...
int main() {
    ...
    test_dma_pull_data_inactive_channel();
